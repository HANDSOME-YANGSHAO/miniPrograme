...
Page({
  ...
  async getVideoNavList() {
    ...
    this.setData({
      videoNavList: res.data.slice(0,14),
      currentNavId: res.data[0].id
    })
    this.getVideoData(res.data[0].id)
  },
  // 获取视频数据(根据视频标签id)
  ...
  changeNav(event) {
    ...
    // 请求新的导航标签视频
    this.getVideoData(this.data.currentNavId)
  },
  // 视频播放的事件回调
  handPlay(event) {
    // 获取当前video的id
...
// 2333这里本来是想做播放当前视频然后暂停上一个video的播放的，但是
// 做用image进行性能优化的时候，点谁就让那个video显示，让其它的video
// 都变成image，这里直接把video都弄没了，其实也就达到了播放当前video
// 暂停上一个video的效果，性能优化真的是一步一步过来的哈哈哈，好好理解！

    // // 如果有上个播放的video上下文对象，让其停止播放，并且上个等于当前这个就部停止
    // this.vid !== vid && this.videoContext && this.videoContext.stop()
    // // 上述判断结束后更新的上下文video组件对象
    // this.vid = vid

    this.videoContext = wx.createVideoContext(vid)
    
    // 这里的作用是：每次播放之前，先根据当前video的id获取到当前
    // video的播放记录时间，如果没有就说明是没有播放记录，或者播放结束
    // 从头开始播放 
    let {videoUpdateTimeMap} = this.data
    if(videoUpdateTimeMap.get(vid)) {
      this.videoContext.seek(videoUpdateTimeMap.get(vid))
    }
  ...
  handleUpdateTime(event) {
    let currentUpdateTime = event.detail.currentTime
    let {videoId, videoUpdateTimeMap} = this.data
    videoUpdateTimeMap.set(videoId, currentUpdateTime)
    this.setData({
      videoUpdateTimeMap
    })
  },
  ...
  handleEnd() { 
    let {videoId, videoUpdateTimeMap} = this.data
    videoUpdateTimeMap.delete(videoId)
    this.setData({
      videoUpdateTimeMap
    })
  },
